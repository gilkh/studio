...
export default function AdminUserManagementPage({ params }: { params: { id: string } }) {
    ...
    const fetchAllData = async () => {
        ...
        try {
            const [userProfile, vendorProfile] = await Promise.all([
                ...
                getVendorProfile(userId) // Attempt to get vendor profile
            ]);

            if (userProfile) {
                setUser(userProfile);
                userForm.reset(userProfile);
            }

            if (vendorProfile) {
                setVendor(vendorProfile);
                vendorForm.reset(vendorProfile);
